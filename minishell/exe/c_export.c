...
/*   Updated: 2023/05/15 19:55:26 by mserrouk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int 	check_export(t_env **envs, t_cmd *cmd)
{
	...
	while (cmd && cmd->cmd[y.j])
	{
		...
		if (!cmd->cmd[y.j + 1] && cmd->cmd[y.j][0] == '\0')
			return (1);
		while (cmd->cmd[y.j] && cmd->cmd[y.j][y.b] && cmd->cmd[y.j][y.b] != '=')
		{
			if (cmd->cmd[y.j][y.b] == '+'
		...
		while (tmp && cmd->cmd[y.j] && y.b > -1)
		{
				...
				&& y.i == y.plus)
			{
				...
				tmp->p = 1;
				tmp->env = ft_strjoin
					(tmp->env, ft_strdup(cmd->cmd[y.j] + y.i + 2));
				y.flg = 1;
			}
			else if (ft_strnstr(tmp->env, cmd->cmd[y.j], ft_strlen(cmd->cmd[y.j])))
			{
				if (tmp->env[ft_strlen(y.ss)] != '\0' && tmp->env[ft_strlen(y.ss)] != '=')
					break ;
				y.flg = 1;
			}
			tmp = tmp->next;
		...
		{
			free(cmd->cmd[y.j]);
			cmd->cmd[y.j] = ft_strjoin(ft_strdup(y.ss), ft_strdup(cmd->cmd[y.j] + y.i + 1));
		}
		free(y.ss);
		if (!y.flg && y.b > -1)
